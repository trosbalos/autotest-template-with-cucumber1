...
public class WebActionSteps {
    ...
    @Тогда("кликнуть по {string}")
    @И("нажата кнопка {string}")
    @И("активирован чекбокс только с {string}")
    ...
    @И("в выпадающем {string} выбрано {string}")
    public void categoryDropDown(String list, String value) {
        SelenideElement element = pageManager
                .getCurrentPage()
                .getElement(list);
        element
        ...
        element2
                .shouldBe(Condition.visible)
                .click();
        LOGGER.info("в выпадающем '{}' выбрано значение '{}'", list, value);
    }
    ...
    @И("в консоль выведено значение названий и цен первых {int} товаров")
    public void namePriceCountPrinter(int x) {
        AvitoPage page = new AvitoPage();
        for (int i = 1; i < x + 1; i++) {

            LOGGER.info("название принтера '{}' цена '{}'", page.getSearchName(i).getText(), page.getPrice(i).getAttribute("content"));
        }
